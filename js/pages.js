...
import { initI18n, setLang, getLang, updateTranslation } from './i18n.js';

const tVal = (obj) => {
  if (typeof obj === 'string') return obj;
  return obj ? obj[getLang()] || obj.id || '' : '';
};
...
async function loadProgramsPage() {
  ...
  programs.forEach((p, i) => {
    ...
    // Inject Translations
    if (p.title && p.title.id) updateTranslation(`prog.${p.slug}.title`, p.title.id, p.title.ja);
    if (p.description && p.description.id) updateTranslation(`prog.${p.slug}.desc`, p.description.id, p.description.ja);
    ...
    card.innerHTML = `
      ...
      <div class="program-list-card__body">
        ...
        <h2 class="program-list-card__title" data-i18n="prog.${p.slug}.title">${pTitle}</h2>
        <p class="program-list-card__desc" data-i18n="prog.${p.slug}.desc">${pDesc}</p>
...
async function loadActivitiesPage() {
  ...
  grid.innerHTML = activities.map((a, i) => {
    ...
    if (a.title && a.title.id) updateTranslation(`act.${a.id}.title`, a.title.id, a.title.ja);
    if (a.summary && a.summary.id) updateTranslation(`act.${a.id}.sum`, a.summary.id, a.summary.ja);
    ...
    return `
      <a href="/kegiatan-detail.html?id=${a.id}" class="kegiatan-card reveal-up" data-category="${a.category}" ${i > 0 ? `style="--delay: ${Math.min(i * 0.1, 0.3)}s"` : ''}>
        ...
        <div class="kegiatan-card__body">
          ...
          <h3 class="kegiatan-card__title" data-i18n="act.${a.id}.title">${aTitle}</h3>
          <p class="kegiatan-card__desc" data-i18n="act.${a.id}.sum">${aSum}</p>
